...
from teacher.discriminator import Discriminator
from module.common import match_features, compute_f0
from module.wavlm import load_wavlm, extract_wavlm_feature

parser = argparse.ArgumentParser(description="train teacher convertor")

parser.add_argument('dataset')
parser.add_argument('-dep', '--decoder-path', default="t_decoder.pt")
parser.add_argument('-disp', '--discriminator-path', default="t_discriminator.pt")
parser.add_argument('-pep', '--pitch-estimator-path', default="t_f0_estimator.pt")
...
PE, Dec, D = load_or_init_models(device)
...
for epoch in range(args.epoch):
    ...
    for batch, wave in enumerate(dl):
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            with torch.no_grad():
                if args.world_pitch_estimation:
                    ...
                    f0 = PE.estimate(spec)
                content = extract_wavlm_feature(wavlm, wave)
            ...
            loss_mel = (mel(wave_recon) - mel(cut_center_wave(wave))).abs().mean()
            loss_feat = D.feat_loss(wave_recon, cut_center_wave(wave))
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            ...
            logits_real = D.logits(cut_center_wave(wave))
            loss_d = 0
